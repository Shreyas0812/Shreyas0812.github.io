...
- `og-image.png` — 1200×630 social/link preview image, matching the site's light theme. Referenced by the `og:image` and `twitter:image` meta tags.
...
Any static host works (GitHub Pages, Netlify, Cloudflare Pages, etc.) — push the folder; no build required. After deploying, set the canonical/live URL and confirm the `og-image.png` is in place so shared links render a preview card.
